...
description: Token optimization best practices for cost-effective Codex usage. Automatically applies efficient file reading, command execution, and output handling strategies. Includes model selection guidance (Opus for learning, Sonnet for development/debugging). Prefers bash commands over reading files.
version: 1.5.0
...
This skill provides token optimization strategies for cost-effective Codex usage across all projects. These guidelines help minimize token consumption while maintaining high-quality assistance.
...
**Myth:** Having many skills in `.Codex/skills/` increases token usage.

**Reality:** Skills use **progressive disclosure** - Codex sees only skill descriptions at session start (~155 tokens for 4 skills). Full skill content loaded only when activated.
...
2. **Low-cost operation** (< 100 lines output)? -> Use Codex context directly
...
2. Is this a LOW-COST operation? (< 100 lines) -> Use Codex context directly
...
- **LOW-COST operations** (< 100 lines) -> Codex context directly
...
By following these guidelines, users can get 5-10x more value from their Codex subscription while maintaining high-quality assistance.
